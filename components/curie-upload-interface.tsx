...
const API_ENDPOINTS = {
  IMAGES:
    "https://capp-imagescurie-dev-001.calmgrass-38ffeef2.eastus2.azurecontainerapps.io",
  NLP: "https://capp-nlpcurie-dev-001.calmgrass-38ffeef2.eastus2.azurecontainerapps.io",
  BIOSIGNALS:
    "https://capp-biosignalscurie-dev-001.calmgrass-38ffeef2.eastus2.azurecontainerapps.io",
};
...
export default function CurieUploadInterface({
  ...
  const [lungInfectionData, setLungInfectionData] =
    useState<LungInfectionData | null>(null);
  ...
  const generateMockLungInfectionData = () => {
    ...
    const totalRatio =
      (Object.values(rightRatio).reduce((a, b) => a + b, 0) +
        Object.values(leftRatio).reduce((a, b) => a + b, 0)) /
      8;
  ...
  const processEcgResponse = async (file: File) => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.BIOSIGNALS}/biosignals/deteccion_arritmias_pacientes/`,
        {
          method: "POST",
          body: formData,
        }
      ...
      {!showResults ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload patient files for AI-powered analysis
            </CardDescription>
          </CardHeader>
            ...
            >
              <TabsList className="grid grid-cols-3 mb-4">
                <TabsTrigger value="xray">
                  X-Ray Image {fileCountsByType.xray > 0 ? "✓" : ""}
                </TabsTrigger>
                <TabsTrigger value="pdf">
                  Medical History {fileCountsByType.pdf > 0 ? "✓" : ""}
                </TabsTrigger>
                <TabsTrigger value="ecg">
                  ECG Signal {fileCountsByType.ecg > 0 ? "✓" : ""}
                </TabsTrigger>
              </TabsList>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold">
              Lung Infection Analysis Results
            </h2>
          ...
          {lungInfectionData && (
            <LungInfectionResults data={lungInfectionData} />
          )}
        </div>
      )}
    </div>
